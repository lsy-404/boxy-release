...
Before reading the session or contacting a service, Boxy opens a small Fluent setup window for the remote service address. The address starts blank unless supplied through a runtime environment variable, command-line option, or domain-style filename suggestion. No remote is built in; the user confirms the editable address before connecting. Closing the window exits. The address must be an HTTPS origin with no credentials, path, query, or fragment; HTTP is allowed only for localhost development.
